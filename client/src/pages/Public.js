...
import { ThumbUp, ThumbDown, ModeComment, AddComment, Close, Edit, Delete } from '@mui/icons-material';
...
export default function Public(props) {
  ...
  const getComments = async (id) => {
    try {
      const response = await userAxios.get(
        `/api/posts/comments/${id}/comments`
      );
  ...
  return (
    <>
      <h1 style={{ textAlign: 'center', marginTop: "25px" }}>USAMIRA ZONE</h1>
        ...
        >
          <div style={{ display: 'flex' }}>
            <div style={{ marginRight: 'auto' }}>
              <h2 style={{ textDecoration: 'underline' }}>{issue.base}, {issue.state}</h2>
          ...
          <>
            <ButtonGroup style={{ gap: '5px' }} variant="contained" size="small">
              <Button
